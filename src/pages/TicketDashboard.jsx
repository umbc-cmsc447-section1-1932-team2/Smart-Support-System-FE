...
import { Search, Filter, Plus, Calendar, Home as HomeIcon, CheckCircle2, Inbox, Ticket, LogOut, ChevronDown } from 'lucide-react';
...
const TicketDashboard = () => {
  ...
  // 1. New State to track which sidebar button is active
  const [currentTab, setCurrentTab] = useState('CLOSED'); // 'ALL', 'OPEN', or 'CLOSED'
  
  const [isFilterOpen, setIsFilterOpen] = useState(false);
  const [sortOrder, setSortOrder] = useState('newest'); 

  // 2. Updated mock data with a mix of OPEN and CLOSED tickets
  const initialTickets = [
    { id: 1, date: "Mar 5", sortDate: "2026-03-05", title: "Unable to authenticate SSH token...", status: "CLOSED", completed: "Mar 8" },
    { id: 2, date: "Mar 3", sortDate: "2026-03-03", title: "New user needs onboarding access", status: "OPEN", completed: "—" }, // Open ticket
    ...
    { id: 4, date: "Mar 1", sortDate: "2026-03-01T09:00", title: "Monitor has dead pixel line down right edge", status: "OPEN", completed: "—" }, // Open ticket
  ];

  // 3. Filter the tickets based on the current tab BEFORE sorting them
  ...
  return (
    <div className="min-h-screen flex bg-offwhite">
      ...
      <aside className="w-72 bg-gray-50 border-r border-gray-200 flex flex-col">
        ...
        <div className="p-6 flex-1">
          ...
          <nav className="flex flex-col gap-2">
            ...
            {/* 4. Sidebar Buttons update the state onClick */}
      ...
      <main className="flex-1 flex flex-col p-10 overflow-y-auto h-screen">
        ...
        {/* Top Action Row (Search, Filter, Logout) remains exactly the same */}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
            {/* 5. Dynamic Header Title based on the Tab */}
          ...
          <table className="w-full text-left">
            ...
            <tbody className="divide-y divide-gray-50">
              {/* 6. Render the DisplayTickets (which is Filtered AND Sorted) */}
          ...
          {/* Empty State (If there are no tickets in that category) */}
